...
exports.handler = async (event) => {
  ...
  const envUrl = env ? 'https://console.moneyblinks.com' : 'http://localhost:3000'
  // TODO implement
  ...
  if (params.type == "autologin") {
    ...
    const Items = await dynamoScanQuery(tableName, filterExpression, expressionAttributeValues);
    return {
      headers: {
        ...event.headers,
        'Access-Control-Allow-Origin': envUrl
    ...
    const objects = await s3.listObjects({ Bucket: 'moneyblinkswallets3125742-pre' }).promise();
    const fileList = [];
    ...
    const objects = await s3.listObjects({ Bucket: 'mbbucket-2022-pre' }).promise();
    const fileList = [];
    objects.Contents.forEach((obj) => {
      const fileInfo = {
        name: obj.Key,
  ...
  async function dynamoScanQuery(tableName, filterExpression, expressionAttributeValues) {
    const results = [];
    ...
    while (true) {
      const parametros = {
        ...
        ExpressionAttributeValues: expressionAttributeValues ? expressionAttributeValues : undefined,
      };
